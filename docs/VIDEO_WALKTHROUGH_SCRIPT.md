# AegisSolv — Official Video Recording & Demonstration Script
...
* **User Perspective / Voiceover:**  
  *"We start by connecting to the Midnight Preview network. AegisSolv connects either via the Lace Midnight wallet extension or through direct client proof sessions, giving institutional counterparties instant zero-friction access to the confidential solvency ledger."*
...
* **Under the title, you will see 4 data source cards:**
  * Card 1: **Solvency Audit** (Grade 790)
  * Card 2: **Custodian PoR** (Reserve 730)
  * Card 3: **RWA Registry** (Asset 760)
  * Card 4: **Custom Sandbox** (Dev Mode)
* **Click card:** **`Solvency Audit`** (1st card on the left).
* **Values automatically populated into the form:**
  * **Composite Solvency Rating:** `790`  
  ...
  * **Verifiable Revenue / Net Cashflow (USD):** `120000` (\$120,000)  
    *Why this value:* Must be $\ge \$50,000$. Proves strong recurring revenue capacity to service debt obligations.
  * **Debt Service Coverage / Leverage Ratio (%):** `28.0` (28%)  
    *Why this value:* Well below the covenant's 40% maximum ceiling. The institutional golden ratio is under 30%.
  * **Asset / Collateral Coverage Buffer (%):** `210` (210%)  
    *Why this value:* Automatically exceeds the 150% minimum solvency covenant.
  * **Confidential Blinding Salt:** Auto-filled with a 256-bit cryptographic witness key.  
...
* **Notice the green badge that lights up:** `CRYPTOGRAPHIC SIGNATURE VERIFIED` with issuer `did:key:z6MkuAuditUS04`.
* **User Perspective / Voiceover:**  
...
* **Visual Result:** Screen smoothly reveals `STEP 2 • VERIFIER & PUBLIC LEDGER AUDIT`:
  ...
  * **Risk Tier:** `Tier A — Investment Grade (Prime)` (cyan glowing text).
  * **Blinded Commitment (ZK Witness):** Shows `0x4bb06f8e4e3a...` with a note: *"Private witness salt kept secret from public view"*.
  * **Settlement Block:** Active.
  * **On-Chain Contract:** Clickable link `0xae6c1533...` that opens the live contract on Midnight Preview Explorer!
...
* **Click button:** **`Apply Verified Solvency to Capital Facility Engine →`** (cyan button).
...
* **Go to:** Left card under **Requested Facility / Credit Line**.
...
* **Why this matters / Hover mouse over the comparison metrics on the right:**
  ...
  * **Required Collateral:** Drops from **180%** (\$90,000 in Maker/Aave) down to **110%** (\$55,000 in AegisSolv).
...
* **In the left form, adjust these values:**
  * **Minimum Solvency Rating:** Drag slider to **`710`** (was 700).  
  ...
  * **Max Debt Service Coverage / DSCR (%):** Adjust slider to **`38%`** (was 40%).
  * **Minimum Asset / Collateral Coverage (%):** Adjust slider to **`160%`** (was 150%).
* **Click button:** **`Commit Covenants to Midnight Preview Ledger`** (cyan button at bottom).
...
* **Visual Result:** A green confirmation box appears:
  ...
  * Displays Tx: `00abddfd924b8e92c4547185d08ea61bb6035ee7155894f71a9bfdf79471b93d72` with a clickable link: **`Audit on Explorer ↗`**.
* **User Perspective / Voiceover:**  
...
* **Screen displays:** Side-by-side comparison of **Private Witness State** vs **Public Consensus State**.
  ...
  * **Right Column (Public Consensus / Midnight Ledger):** Points out Verified Boolean (`true`), Assigned Tier (`Tier A`), Settlement Block, and Contract Address (`0xae6c1533...`).
